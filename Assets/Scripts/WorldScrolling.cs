...
public class WorldScrolling : MonoBehaviour
{
   ...
   [SerializeField] Vector2Int playerTilePosition;
    ...
    private void UpdateTilesOnScreen()
    {
        for (int pov_x = -(fieldOfVisionWidth / 2); pov_x <= fieldOfVisionWidth / 2; pov_x++) 
        {
            for (int pov_y = -(fieldOfVisionHeight / 2); pov_y <= fieldOfVisionHeight / 2; pov_y++) 
            {
                ...
                Vector3 newPosition= CalculatTilePosition(playerTilePosition.x + pov_x, playerTilePosition.y + pov_y);
    ...
    public void Add(GameObject tilegGameobject, Vector2Int tilePosition)
    {
        terrainTiles[tilePosition.x, tilePosition.y] = tilegGameobject;
    }
